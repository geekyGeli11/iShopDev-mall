    ...
    <div class="table-container">
      <el-table ref="returnApplyTable"
        ...
        <el-table-column label="退款金额" width="120" align="center">
          <template slot-scope="scope">￥{{scope.row.returnAmount || (scope.row | formatReturnAmount)}}</template>
        </el-table-column>
        <el-table-column label="售后类型" width="100" align="center">
          <template slot-scope="scope">
            <el-tag :type="scope.row.returnType === 1 ? 'warning' : 'danger'" size="small">
              {{ scope.row.returnType === 1 ? '仅退款' : '退货退款' }}
            </el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column label="退款状态" width="100" align="center">
          <template slot-scope="scope">
            <el-tag :type="scope.row.refundStatus === 1 ? 'success' : 'info'" size="small">
              {{ scope.row.refundStatus === 1 ? '已退款' : '未退款' }}
            </el-tag>
          </template>
        </el-table-column>
  ...
  export default {
    ...
    created(){
      ...
      if (this.$route.query.status !== undefined) {
        ...
        const statusText = ['待处理', '退货中', '已完成', '已拒绝'][this.listQuery.status];
        this.$message.success(`已自动筛选${statusText}退货申请`);
      }
      this.getList();
    },
    filters:{
