...
<script>
  ...
  export default {
    ...
    created() {
      // 检查路由参数，如果有即将到期的筛选条件则自动设置
      if (this.$route.query.expiringSoon === 'true' && this.$route.query.endDate) {
        this.listQuery.endTime = this.$route.query.endDate;
        this.$message.success('已自动筛选即将到期的广告');
      }
      this.getList();
