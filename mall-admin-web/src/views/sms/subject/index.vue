  ...
  <div class="app-container">
    ...
    <div class="table-container">
      <el-table ref="newSubjectTable" :data="list" style="width: 100%;" @selection-change="handleSelectionChange"
        ...
        <el-table-column label="专题封面" width="120" align="center">
          <template slot-scope="scope">
            <img :src="scope.row.cover" alt="封面图" style="width: 100px; height: 60px; object-fit: cover;" />
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="180" align="center">
          <template slot-scope="scope">
            <el-button size="mini" type="text" @click="handleEditSort(scope.$index, scope.row)">编辑
            </el-button>
    ...
    <el-dialog title="编辑" :visible.sync="sortDialogVisible" width="40%">
      <el-form :model="sortDialogData" label-width="150px">
        <el-form-item label="排序：">
          <el-input v-model="sortDialogData.sort" style="width: 200px"></el-input>
        </el-form-item>
        <el-form-item label="封面图：">
          <single-upload v-model="sortDialogData.cover"></single-upload>
        </el-form-item>
        <el-form-item label="专题相册：">
          <multi-upload v-model="processedPicsAlbum"></multi-upload>
        </el-form-item>
        <el-form-item label="专题描述：">
          <el-input v-model="sortDialogData.subjectDescription" type="textarea" :autosize="{ minRows: 3, maxRows: 5 }"
...
export default {
  ...
  data() {
    return {
      ...
      sortDialogData: { sort: 0, cover: null, id: null, subjectDescription: null, picsAlbum: null }
    }
  },
  computed: {
    // 获取和设置 sortDialogData 中的 picsAlbum
    processedPicsAlbum: {
      get() {
        // 如果 this.sortDialogData.picsAlbum 是字符串，转换为数组
        if (
          this.sortDialogData.picsAlbum === undefined ||
          this.sortDialogData.picsAlbum === null ||
          this.sortDialogData.picsAlbum === ''
        ) {
          return [];
        }
        // 如果是逗号分隔的字符串，分割成数组
        return this.sortDialogData.picsAlbum.split(',');
      },
      set(newValue) {
        // 如果新值是数组，转回逗号分隔的字符串
        if (newValue === null || newValue.length === 0) {
          this.sortDialogData.picsAlbum = '';  // 设置为空
        } else {
          this.sortDialogData.picsAlbum = newValue.join(',');  // 数组转为逗号分隔字符串
        }
      }
    }
  },
  ...
  methods: {
    ...
    handleEditSort(index, row) {
      ...
      this.sortDialogData.cover = row.cover;
      this.sortDialogData.subjectDescription = row.subjectDescription;
      this.sortDialogData.picsAlbum = row.picsAlbum;
