...
@Slf4j
@Service
public class WxPayBusinessImpl implements WxPayBusiness {
    @Autowired
    ...
    public String parseOrderNotifyResult(String xmlData) throws WxPayException {
        try {
            ...
            if (orderSn == null || orderSn.isEmpty()) {
                throw new WxPayException("订单号缺失");
            }

            // 统一使用paySuccessByOrderSn处理支付成功逻辑
            // 这样可以确保库存处理、送礼记录等逻辑的一致性
...
//    @PostMapping("/notify/scanpay")
    ...
    @Override
    public Integer paySuccess(Long orderId, Integer payType, String giftMessage, String giftPic) {
        // 统一使用portalOrderService.paySuccess处理支付成功逻辑
