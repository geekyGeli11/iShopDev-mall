...
public class SmsHomeRecommendProductController {
    ...
    @Operation(summary = "修改推荐商品")
    @RequestMapping(value = "/update/sort/{id}", method = RequestMethod.POST)
    @ResponseBody
    public CommonResult updateSort(@PathVariable Long id, Integer sort, String cover, Long schoolId) {
        int count = recommendProductService.updateSort(id, sort, cover, schoolId);
        if (count > 0) {
            return CommonResult.success(count);
        }
        return CommonResult.failed();
    ...
    public CommonResult<CommonPage<SmsHomeRecommendProduct>> list(@RequestParam(value = "productName", required = false) String productName,
        ...
        List<SmsHomeRecommendProduct> homeBrandList = recommendProductService.list(productName, recommendStatus, schoolId, pageSize, pageNum);
