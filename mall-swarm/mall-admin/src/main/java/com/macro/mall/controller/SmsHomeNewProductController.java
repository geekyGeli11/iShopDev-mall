...
public class SmsHomeNewProductController {
    ...
    public CommonResult updateSort(@PathVariable Long id, Integer sort, String cover, Long schoolId) {
        int count = homeNewProductService.updateSort(id, sort, cover, schoolId);
        if (count > 0) {
            return CommonResult.success(count);
        }
        return CommonResult.failed();
    ...
    public CommonResult<CommonPage<SmsHomeNewProduct>> list(@RequestParam(value = "productName", required = false) String productName,
        ...
        List<SmsHomeNewProduct> homeBrandList = homeNewProductService.list(productName, recommendStatus, schoolId, pageSize, pageNum);
