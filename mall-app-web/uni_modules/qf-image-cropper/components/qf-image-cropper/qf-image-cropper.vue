<template>
	<view class="image-cropper" :style="{ zIndex }" @wheel="cropper.mousewheel">
		...
		<slot />
		<view v-if="showBottomBar" class="fixed-bottom safe-area-inset-bottom" :style="{ zIndex: initData.area.zIndex + 99 }">
...
<script>
	...
	export default {
		...
		props: {
			...
			/** 图片初始显示宽度所占屏幕宽度百分比（用于控制图片初始大小，独立于裁剪框大小） */
			imageWidthPercent: {
				type: Number,
				default: 0  // 0表示使用裁剪框大小
			},
			/** 图片初始显示高度所占屏幕宽度百分比（用于控制图片初始大小，独立于裁剪框大小） */
			imageHeightPercent: {
				type: Number,
				default: 0  // 0表示使用裁剪框大小
			},
		...
		methods: {
			...
			initArea() {
				...
				// 如果不显示底部栏，则高度为0
				this.sys.offsetBottom = this.showBottomBar ? (uni.upx2px(100) + this.sys.safeAreaInsets.bottom) : 0;
				// #ifndef H5
				...
				// 计算图片初始显示大小（可以独立于裁剪框大小）
				if (this.imageWidthPercent > 0 && this.imageHeightPercent > 0) {
					// 如果指定了图片显示百分比，使用指定值
					let iwp = this.imageWidthPercent;
					let ihp = this.imageHeightPercent;
					if (this.imgWidth > this.imgHeight) {
						ihp = ihp * this.imgHeight / this.imgWidth;
					} else if (this.imgWidth < this.imgHeight) {
						iwp = iwp * this.imgWidth / this.imgHeight;
					}
					this.scaleWidth = size * iwp / 100;
					this.scaleHeight = size * ihp / 100;
				} else {
					// 否则使用裁剪框大小
